...
namespace MyCompany.RevitCommands
{
  ...
  public class DrawRevisionCloudAroundSelectedElementsInSheetsAndInheritRevision : IExternalCommand
  {
      ...
      ElementSet elements)
    {
      ...
      //------------------------------------------------------------------
      // 6) Report skipped items and sheets, if any
      //------------------------------------------------------------------
      List<string> messages = new List<string>();

      if (failedElems.Count > 0)
      {
        messages.Add($"{failedElems.Count} of {selIds.Count} selected element(s) " +
          "are in Drafting/Legend views or not visible on any chosen sheet.");
      }

      if (skippedSheets > 0)
      {
        messages.Add($"{skippedSheets} sheet(s) were skipped because they have no revisions: " +
          string.Join(", ", skippedSheetNumbers.Take(5)) + 
          (skippedSheetNumbers.Count > 5 ? "..." : ""));
      }

      if (messages.Count > 0)
      {
        TaskDialog.Show("Revision Clouds", string.Join("\n\n", messages));
      }
